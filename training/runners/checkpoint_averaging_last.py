...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        nargs=1,
        help='Input folder that contains only one .nemo file',
    )
    parser.add_argument(
        '--import_fname_list',
        metavar='FILE',
    ...
    parser.add_argument(
        '--ntk_ckpts',
        metavar='Number of topk checkpoints to be averaged across',
        type=int,
        nargs=1,
        default=5
    )
    ...
    # for model_fname_i, model_fname in enumerate(
    
    nemo_files = glob.glob(f"{args.model_fname_dir[0]}/**/*.nemo",recursive=True)
    ...
    # logging.info(f"\n===> [{model_fname_i+1} / {len(args.model_fname_dir)}] Parsing folder {model_folder_path}\n")
    ...
    # load state dicts
    n = min(args.ntk_ckpts,len(checkpoint_paths))

    checkpoint_paths_sorted=sorted(checkpoint_paths,key=lambda x: int(x.split('-epoch=')[-1].split('.')[0]),reverse=True) # low wer to high wer
    checkpoint_paths = checkpoint_paths_sorted[:n]
